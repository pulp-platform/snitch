...
static inline void pulp_ssr_read(enum ssr_dm dm, enum ssr_dim dim, volatile void *ptr) {
    ssr_config_reg[dm].rptr[dim].value = (uint32_t)ptr;
}

/// Start a streaming write.
static inline void pulp_ssr_write(enum ssr_dm dm, enum ssr_dim dim, volatile void *ptr) {
    ssr_config_reg[dm].wptr[dim].value = (uint32_t)ptr;
}

/// Synchronize the integer and float pipelines.
static inline void fpu_fence() {
    asm volatile ("fmv.x.w zero, fa0");
}
