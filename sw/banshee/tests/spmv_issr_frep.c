...
static inline void svec16_dvec_spmv_naive(
...
static inline void svec16_dvec_spmv_opt_issr(
    ...
    if (rows_a == 0) return;
    uint32_t* rlst = ptrs_a + rows_a;
    uint32_t roffs_first = *ptrs_a;
    uint32_t roffs_last = *rlst;
...
int main() {
    ...
    // Run kernels
    svec16_dvec_spmv_naive(a_vals, a_idcs, a_ptrs, A_ROWS, b_vals, res_gcc);
    svec16_dvec_spmv_opt_issr(a_vals, a_idcs, a_ptrs, A_ROWS, b_vals, res_issr,
                              1);
    // Check results
    int errors = A_ROWS;
