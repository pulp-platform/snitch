...
impl<'a> ElfTranslator<'a> {
    ...
    unsafe fn translate_inner(&mut self) -> Result<()> {
        ...
        let rpt_ptr_ref = LLVMBuildAlloca(builder, LLVMInt32Type(), b"frep_rpt_ptr\0".as_ptr() as *const _);
        LLVMBuildStore(builder, const_zero_32, rpt_ptr_ref);
        let max_rpt_ref = LLVMBuildAlloca(builder, LLVMInt32Type(), b"frep_max_rpt\0".as_ptr() as *const _);
