...
void softmax_layer(const softmax_layer_t *l) {
    ...
    uint32_t ifmap_size = l->BATCH_SIZE * l->SEQ_LEN * l->INPUT_SAMPLES * sizeof(float);
    ...
    if (snrt_is_compute_core() &&
        ...
        softmax_fp32(&ifmap[row_offset], &ofmap[row_offset], ldI, batch_offset, l->BATCH_SIZE, l->SEQ_LEN / 8, l->INPUT_SAMPLES);
