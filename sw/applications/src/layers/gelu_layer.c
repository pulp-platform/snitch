...
void gelu_layer(const gelu_layer_t *l) {
    ...
    uint32_t ifmap_size = l->BATCH_SIZE * l->SEQ_LEN * l->HIDDEN_NODES * sizeof(float);
    ...
    if (snrt_is_compute_core() &&
        ...
        for (int b = 0; b < l->BATCH_SIZE; b++) {
            ...
            //     printf("BATCH: %d\n", b);
            // }
            gelu_fp32(&ifmap[row_offset + b * batch_offset], &ofmap[row_offset + b * batch_offset], ldI, l->BATCH_SIZE, l->SEQ_LEN / 8, l->HIDDEN_NODES);
        }
