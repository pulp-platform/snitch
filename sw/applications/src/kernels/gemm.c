...
void gemm_fp64_opt(uint32_t M, uint32_t N, uint32_t K, double* A, uint32_t ldA,
    ...
    for (int i = 0; i < M * N / unroll; i++) {
        asm volatile(
            // First iteration is done with fmul.d to
            // prevent zero initialization
            "fmul.d %[c0], ft0, ft1 \n"
            "fmul.d %[c1], ft0, ft1 \n"
            "fmul.d %[c2], ft0, ft1 \n"
            "fmul.d %[c3], ft0, ft1 \n"
            // perform frep loop with fmadd.d
            "frep.o %[n_frep], 4, 0, 0 \n"
            "fmadd.d %[c0], ft0, ft1, %[c0] \n"
            "fmadd.d %[c1], ft0, ft1, %[c1] \n"
            "fmadd.d %[c2], ft0, ft1, %[c2] \n"
            "fmadd.d %[c3], ft0, ft1, %[c3] \n"
            // Store back last result
