...
module fixture_ssr_streamer import snitch_ssr_pkg::*; #(
  ...
  task automatic cfg_write_regs (input logic [6:0] ssr, input cfg_regs_t cfg);
    ...
    cfg_write(ssr, 10, cfg.idx_size);
    cfg_write(ssr, 11, cfg.idx_base);
    cfg_write(ssr, 12, cfg.idx_shift);
  endtask
  ...
  task automatic verify_isect_inout (
    ...
    regs  [2] = {32'h0, 32'(data_base[2]), 32'(idx_size[2]),
    ...
    // Master 0
    regs  [0] = {32'h0, 32'(data_base[0]), 32'(idx_size[0]),
                  (32*4)'(WordBytes), (32*4)'(idx_bound[0]), 32'h0};
    status[0] = '{upper: {1'b0, 1'b0, {1'b1, merge}, 1'b1}, ptr: idx_base[0]};
    verify_launch(0, regs[0], status[0], alias_launch);
    // Master 1
    regs  [1] = {32'h0, 32'(data_base[1]), 32'(idx_size[1]),
                  (32*4)'(WordBytes), (32*4)'(idx_bound[1]), 32'h0};
    status[1] = '{upper: {1'b0, 1'b0, {1'b1, merge}, 1'b1}, ptr: idx_base[1]};
